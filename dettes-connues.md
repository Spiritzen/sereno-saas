...
### Dette n°47 — Factory facture : numéros émis probabilistes
- **Sévérité** : (résolue — était BASSE, instabilité CI uniquement, aucun risque en production)
- **Statut** : RÉSOLUE — micro-sprint T0 dédié, 20/08/2026 (branche `fix/tests-factory-factures-numero-unique`)
- **Preuve** : une exécution GitHub Actions de la PR de l'entrée publique a échoué une fois (`ActiveRecord::RecordNotUnique` / `PG::UniqueViolation` sur `index_factures_unique_numero_by_org`, clé `(organisation_id, numero)=(..., FAC-2026-6250)`), puis est repassée au vert à la relance sans changement de code — signature d'un défaut probabiliste. Cause : les traits `:emise` et `:deposee` de `backend/spec/factories/factures.rb` tiraient chacun `numero = "FAC-#{Date.current.year}-#{format('%04d', rand(1000..9999))}"` dans leur `after(:create)` ; avec assez de créations pour une même organisation dans un même processus de test (ex. 12 factures `:emise` dans `backend/spec/requests/destinataire/factures_spec.rb`), une collision devenait plausible (paradoxe des anniversaires sur un espace de 9000 valeurs). Corrigé par une séquence FactoryBot monotone et déterministe, `sequence(:numero_facture_test) { |n| "FAC-#{Date.current.year}-#{n.to_s.rjust(4, '0')}" }`, déclarée à la racine de `FactoryBot.define` (pas dans le bloc `factory :facture`, sans quoi FactoryBot la traite comme un attribut implicite de la factory). Preuve permanente ajoutée : `backend/spec/models/facture_factory_spec.rb` (3 exemples, 30+25+20 créations), qui démontre que `:emise` et `:deposee` produisent toujours des `numero` distincts pour une même organisation, y compris entre les deux traits. Mutation négative effectuée : une valeur constante temporaire fait échouer les 3 exemples avec `PG::UniqueViolation` (la contrainte PostgreSQL reste active et détecte réellement la collision), restaurée immédiatement, suite repassée au vert. L'exemple historiquement instable (`factures_spec.rb:408`) a été rejoué 10 fois consécutives, toutes vertes. Suite backend complète : 712 exemples, 0 échec.
- **Impact / pourquoi c'était une dette** : un tirage aléatoire dans un espace fini n'est jamais une garantie d'unicité — PostgreSQL protégeait correctement l'invariant métier, c'est la factory de test qui violait son propre contrat. Aucun risque de confidentialité ni de régression fonctionnelle de l'application (l'index et la génération réelle du numéro via `Numerotation`/`NumerotationService` étaient déjà corrects) — uniquement une source de flakiness CI, désormais éliminée sans jamais assouplir ni masquer la contrainte PostgreSQL.
- **Quand ça a été traité** : micro-sprint T0 dédié du 20/08/2026, correction strictement limitée à l'infrastructure de test (aucun fichier de production touché).
